 ...
 *   https://datatables.net/download/#bs4/dt-1.10.23/b-1.6.5/b-colvis-1.6.5/b-flash-1.6.5/b-print-1.6.5/kt-2.5.3/r-2.2.6/sc-2.0.3/sp-1.2.2/sl-1.3.1
 *
 * Included libraries:
 *   DataTables 1.10.23, Buttons 1.6.5, Column visibility 1.6.5, Flash export 1.6.5, Print view 1.6.5, KeyTable 2.5.3, Responsive 2.2.6, Scroller 2.0.3, SearchPanes 1.2.2, Select 1.3.1
 */
	...
	(function ($, window, document, undefined) {
		...
		function _fnFeatureHtmlFilter(settings) {
			...
			var jqFilter = $('input', filter)
				...
				.attr('placeholder', 'cari nama,kelas,dan username')
				.css({
					'width': '250px',
					'height': '35px',
					'font-size': '15px',
		...
		DataTable.ext = _ext = {
			 ...
			 *  @type string
			 */
			build: "bs4/dt-1.10.23/b-1.6.5/b-colvis-1.6.5/b-flash-1.6.5/b-print-1.6.5/kt-2.5.3/r-2.2.6/sc-2.0.3/sp-1.2.2/sl-1.3.1",
	...
	var SearchPane = /** @class */ (function () {
		...
		SearchPane.prototype._addRow = function (display, filter, shown, total, sort, type, className) {
			...
			return this.s.dtPane.row.add({
				className: className,
				display: display !== '' ?
					display : this.s.colOpts.emptyMessage !== false ?
					this.s.colOpts.emptyMessage : this.c.emptyMessage,
				filter: filter,
				index: index,
				shown: shown,
				sort: sort !== '' ?
					sort : this.s.colOpts.emptyMessage !== false ?
					this.s.colOpts.emptyMessage : this.c.emptyMessage,
				total: total,
				type: type
			});
			...
			this.s.dtPane = $(this.dom.dtP).DataTable($.extend(true, {
					columnDefs: [{
							...
							type: table.settings()[0].aoColumns[this.s.index] !== undefined ?
								table.settings()[0].aoColumns[this.s.index]._sManualType : null
						},
						{
			...
			if (!this.s.dt.page.info().serverSide &&
				...
				if (this.colExists) {
					...
					//  there is a need to update certain parts ofthe rowData.
						...
						this.s.dt.rows().data().toArray().length === this.s.dt.rows({
							search: 'applied'
						}).data().toArray().length) {
