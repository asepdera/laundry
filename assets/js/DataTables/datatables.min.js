...
$jscomp.SIMPLE_FROUND_POLYFILL = !1;
$jscomp.ISOLATE_POLYFILLS = !1;
$jscomp.defineProperty = $jscomp.ASSUME_ES5 || "function" == typeof Object.defineProperties ? Object.defineProperty : function (m, t, h) {
...
$jscomp.polyfillIsolated = function (m, t, h, r) {
	...
	null != t && (m ? $jscomp.defineProperty($jscomp.polyfills, v, {
		...
		writable: !0,
		value: t
	}) : t !== h && ($jscomp.propertyToPolyfillSymbol[v] = $jscomp.IS_SYMBOL_NATIVE ? $jscomp.global.Symbol(v) : $jscomp.POLYFILL_PREFIX + v, v =
...
$jscomp.polyfill("Symbol.iterator", function (m) {
		...
		for (var t = "Array Int8Array Uint8Array Uint8ClampedArray Int16Array Uint16Array Int32Array Uint32Array Float32Array Float64Array".split(" "), h = 0; h < t.length; h++) {
...
(function () {
	...
	var h, r, v = function () {
			function k(a, b, c, d, e, g) {
				...
				this.s.searchFunction = function (n, x, z, y) {
					...
					n = null;
					f.colExists && (n = x[f.s.index], "filter" !== l.orthogonal.filter && (n = f.s.rowData.filterMap.get(z), n instanceof h.fn.dataTable.Api && (n = n.toArray())));
			...
			k.prototype.destroy =
				function () {
					...
					for (var a = h.fn.dataTable.ext.search.indexOf(this.s.searchFunction); - 1 !== a;) h.fn.dataTable.ext.search.splice(a, 1), a = h.fn.dataTable.ext.search.indexOf(this.s.searchFunction);
				...
				if (this.colExists) {
					...
					if (!this.s.dt.page.info().serverSide && (null === c || null ===
						...
						if (void 0 !== c.searchPanes.options[b])
							for (u = 0, b = c.searchPanes.options[b]; u < b.length; u++) w = b[u], this.s.rowData.arrayFilter.push({
								display: w.label,
								filter: w.value,
								sort: w.label,
								type: w.label
							}), this.s.rowData.bins[w.value] = this.c.viewTotal || this.c.cascadePanes ? w.count : w.total, this.s.rowData.binsTotal[w.value] = w.total;
						u = Object.keys(n.binsTotal).length;
						b = this._uniqueRatio(u,
							this.s.tableLength);
						if (!1 === this.s.displayed && ((void 0 === p.show && null === p.threshold ? b > this.c.threshold : b > p.threshold) || !0 !== p.show && 1 >= u)) {
							this.dom.container.addClass(this.classes.hidden);
							this.s.displayed = !1;
							return
						}
			...
			k.prototype._getComparisonRows = function () {
				...
				if (void 0 !== a) {
					...
					for (var f = 0; f < a.length; f++) {
						...
						}(!this.c.cascadePanes || this.c.cascadePanes && 0 !== y) && g.push(this._addRow(x, z, y, u, w, p, n))
					}
					return g
				}
			};
			...
			k.prototype._updateCommon = function (a) {
				...
				if (!(this.s.dt.page.info().serverSide || void 0 === this.s.dtPane || this.s.filteringActive && !this.c.cascadePanes && !0 !== a || !0 === this.c.cascadePanes &&
					...
					this.s.dtPane.clear();
					if (this.colExists) {
						0 === d.arrayFilter.length ? this._populatePane() : this.c.cascadePanes && this.s.dt.rows().data().toArray().length === this.s.dt.rows({
							search: "applied"
						}).data().toArray().length ? (d.arrayFilter = d.arrayOriginal, d.bins = d.binsOriginal) : (this.c.viewTotal ||
		...
		q, A, G = function () {
			function k(a, b, c) {
				...
				if (void 0 === e.settings()[0]._searchPanes) {
					...
					e.on("xhr", function (g, f, l, p) {
						l && l.searchPanes && l.searchPanes.options && (d.s.serverData = l, d.s.serverData.tableLength = l.recordsTotal, d._serverTotals())
					});
			...
			k.prototype.rebuild = function (a, b) {
				...
				for (var d =
					...
					var g = e[d];
					if (!1 === a || g.s.index === a) g.clearData(), c.push(g.rebuildPane(void 0 !== this.s.selectionList[this.s.selectionList.length - 1] ? g.s.index === this.s.selectionList[this.s.selectionList.length - 1].index : !1, this.s.dt.page.info().serverSide ? this.s.serverData : void 0, null, b)), q(this.dom.panes).append(g.dom.container)
				}
					...
					if (this.regenerating) {
						...
						for (e = this.s.panes; a < e.length; a++)
							if (f = e[a], void 0 !== f.s.dtPane) {
								...
								if (-1 !== d && null !== d && d === f.s.index || !1 === c || f.s.index === g) b = !1, f.s.filteringActive = !1;
								f.updatePane(b ? c : b)
							} this._updateFilterCount()
					} else {
						l = 0;
			...
			k.prototype._attach = function () {
				...
				this.c.clear && (q(this.dom.clearAll).appendTo(this.dom.titleRow), q(this.dom.clearAll).on("click.dtsps", function () {
					a.clearSelections()
				}));
							...
							if (f.s.index === a[b].index && void 0 !== f.s.dtPane) {
								...
								for (var l = function (x) {
										f.s.dtPane.rows().every(function (z) {
											void 0 !== f.s.dtPane.row(z).data() && void 0 !== x && f.s.dtPane.row(z).data().filter === x.filter && f.s.dtPane.row(z).select()
										})
			...
			k.prototype._paneDeclare = function (a, b, c) {
				...
				if (0 < this.c.order.length)
					for (e = this.c.order.map(function (l, p, n) {
						...
						}), this.dom.panes.empty(), this.s.panes = e, e = 0, g = this.s.panes; e < g.length; e++) this.dom.panes.append(g[e].dom.container);
			...
			k.prototype._serverTotals = function () {
				...
				if (a) {
					...
					for (d = 0; d < this.s.selectionList.length; d++) {
						g = !1;
						for (e = d + 1; e < this.s.selectionList.length; e++) this.s.selectionList[e].index === this.s.selectionList[d].index && (g = !0);
						if (!g) {
							e = !1;
							a = 0;
							for (var f = this.s.panes; a < f.length; a++) g = f[a], g.s.index === this.s.selectionList[d].index && 0 < g.s.dtPane.rows({
								selected: !0
				...
				if (b && 1 === this.s.selectionList.length)
					for (b =
						0, d = this.s.panes; b < d.length; b++) g = d[b], g.s.lastSelect = !1, g.s.deselect = !1, void 0 !== g.s.dtPane && 0 < g.s.dtPane.rows({
						selected: !0
	...
	(function (k) {
		...
		k(b).on("preInit.dt.dtsp", function (e, g, f) {
			"dt" === e.namespace && (g.oInit.searchPanes || d.defaults.searchPanes) && (g._searchPanes || c(g, !0))
		});
		d.ext.feature.push({
			cFeature: "P",
			fnInit: c
