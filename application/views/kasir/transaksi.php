...
<body>
    <main class="main-content">
        ...
        <form action="<?= site_url('masukanTransaksiKasir') ?>" method="post" id="formTransaksi">
            ...
            <div class="row">
                ...
                <div class="col-md-12 mt-3 p-3" style="background: #fff;">
                    ...
                    <div class="mt-3">
                        <input type="text" name="pajak" class="form-control angka" placeholder="Pajak">
                    </div>
                    <div class="mt-3">
                        <input type="text" name="biaya_lain" class="form-control angka" placeholder="Biaya lain">
                    </div>
                    <div class="mt-3">
                        <input type="text" name="diskon" class="form-control angka" placeholder="Diskon" id="diskon">
                    </div>
    ...
    <script>
        ...
        ?>
        $.ajax({
            url: 'http://localhost/laundry/index.php/ambilDataMemberKasir',
            method: 'GET',
            dataType: 'JSON',
            success: data => {
        ...
        $.ajax({
            url: `http://localhost/laundry/index.php/dataKasir?id_outlet=<?= $this->session->userdata('id_outlet') ?>`,
            method: 'GET',
            dataType: 'JSON',
            success: data => {
        ...
        $('#cari-member').keyup(e => {
            ...
            if (e.target.value == '') {
                ...
                $.ajax({
                    url: `http://localhost/laundry/index.php/cariDataMemberKasir/${e.target.value}`,
                    method: 'GET',
                    dataType: 'JSON',
                    success: data => {
                        for (let i = 0; i < data.length; i++) {
        ...
        $('#cari-paket').keyup(e => {
            ...
            if (e.target.value == '') {
                ...
                $.ajax({
                    url: `http://localhost/laundry/index.php/cariDataPaketKasir/${e.target.value}/${'<?= $this->session->userdata('id_outlet') ?>'}`,
                    method: 'GET',
                    dataType: 'JSON',
                    success: data => {
                        for (let i = 0; i < data.length; i++) {
        ...
        const dapetinDataPaket = id => {
            $.ajax({
                url: `http://localhost/laundry/index.php/dataKasir?id=${id}`,
                method: 'GET',
                dataType: 'JSON',
                success: data => {
        ...
        const bertambah = (h, e, i, ik) => {
            ...
            nilai++
            //ubah nilai dari input qty sesuai increment
            e.value = nilai
            //harga * qty
            jumlah = h * nilai
